...
- Your program must validate the user's input and catch any errors.
  - If the user inputs an expression which is not well-formed (for example missing inputs, contains negative values, etc.), your program will report the error message `malformed expression` followed by a new line and end immediately with a return value of `1`. 
  - If the user inputs an expression with an invalid item (not on the menu, out of stock, etc.), your program will report the error message `invalid item` followed by a new line and end immediately with a return value of `2`.  
